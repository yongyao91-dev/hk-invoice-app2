...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      const { data, error } = await supabase.from("invoices").select("*").order("saved_at", { ascending: false });
    ...
    if (req.method === "POST") {
      ...
      const { data, error } = await supabase.from("invoices").insert([{
        outlet: invoice.outlet, supplier: invoice.supplier,
        invoice_no: invoice.invoice_no, date: invoice.date,
        items: invoice.items, total: invoice.total,
        notes: invoice.notes, payment_status: invoice.payment_status || "unpaid",
